...
@section('title', __('messages.home') . ' — SeasideWorld')
    ...
    <div class="container">
        <div class="hero-content">
            <div class="hero-badge">
                ...
                {{ __('messages.hero_badge') ?? 'Trusted Since 2005' }}
            </div>
            <h1 class="hero-title">
                {{ __('messages.hero_title') ?? 'Premium Japanese Vehicles,' }}<br>
                <span>{{ __('messages.hero_title_2') ?? 'Exported Globally' }}</span>
            </h1>
            <p class="hero-subtitle">
                {{ __('messages.hero_subtitle') ?? 'Discover an extensive selection of quality Japanese cars, trucks, and SUVs. We handle everything — sourcing, inspection, documentation and shipping to your door.' }}
            </p>
            <div class="hero-cta-group">
                <a href="{{ route('gallery') }}" class="btn-hero-primary">
                    <i class="fa fa-car me-2"></i>{{ __('messages.view_gallery') ?? 'View Gallery' }}
                </a>
                <a href="{{ route('contact') }}" class="btn-hero-outline">
                    <i class="fa fa-envelope me-2"></i>{{ __('messages.contact_us') ?? 'Contact Us' }}
                </a>
            </div>
        </div>
    </div>
...
<section class="features-section">
    <div class="container">
        <div class="text-center mb-5">
            <h2 class="section-title">{{ __('messages.why_choose_us') ?? 'Why Choose SeasideWorld?' }}</h2>
            <p class="section-subtitle">{{ __('messages.why_subtitle') ?? 'We provide end-to-end vehicle export solutions with unmatched quality and service.' }}</p>
        </div>
        <div class="row g-4">
            <div class="col-sm-6 col-lg-3">
                <div class="feature-card">
                    <div class="feature-icon"><i class="fa fa-shield-alt"></i></div>
                    <h5>{{ __('messages.feature_quality') ?? 'Quality Assured' }}</h5>
                    <p>{{ __('messages.feature_quality_desc') ?? 'Every vehicle undergoes rigorous multi-point inspection before export.' }}</p>
                </div>
            </div>
            <div class="col-sm-6 col-lg-3">
                <div class="feature-card">
                    <div class="feature-icon"><i class="fa fa-ship"></i></div>
                    <h5>{{ __('messages.feature_shipping') ?? 'Global Shipping' }}</h5>
                    <p>{{ __('messages.feature_shipping_desc') ?? 'We ship to over 80 countries worldwide with reliable logistics partners.' }}</p>
                </div>
            </div>
            <div class="col-sm-6 col-lg-3">
                <div class="feature-card">
                    <div class="feature-icon"><i class="fa fa-headset"></i></div>
                    <h5>{{ __('messages.feature_support') ?? '24/7 Support' }}</h5>
                    <p>{{ __('messages.feature_support_desc') ?? 'Our multilingual team is available around the clock to assist you.' }}</p>
                </div>
            </div>
            <div class="col-sm-6 col-lg-3">
                <div class="feature-card">
                    <div class="feature-icon"><i class="fa fa-file-signature"></i></div>
                    <h5>{{ __('messages.feature_docs') ?? 'Paperwork Handled' }}</h5>
                    <p>{{ __('messages.feature_docs_desc') ?? 'We manage all export documentation, customs clearance, and registration.' }}</p>
                </div>
            </div>
        </div>
    </div>
...
<section class="vehicles-section">
    <div class="container">
        <div class="text-center mb-5">
            <h2 class="section-title">{{ __('messages.featured_vehicles') ?? 'Featured Vehicles' }}</h2>
            <p class="section-subtitle">{{ __('messages.featured_subtitle') ?? 'Explore our latest selection of premium Japanese vehicles ready for export.' }}</p>
        </div>
        ...
        <div class="row g-4">
                    ...
                    <div class="vehicle-card card h-100">
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="d-flex align-items-center gap-2 text-muted mb-3" style="font-size: 0.85rem;">
                                <span><i class="fa fa-calendar-alt me-1"></i>{{ $vehicle->year }}</span>
                            </div>
                            @if($vehicle->price)
                            ...
                            @endif
                            <a href="{{ route('gallery') }}" class="btn btn-primary mt-auto w-100 btn-sm">
            ...
            @empty
                <div class="col-12 text-center py-5">
                    ...
                    <a href="{{ route('gallery') }}" class="btn btn-primary">Browse All Vehicles</a>
                </div>
            @endforelse
        </div>
        ...
        @if(isset($featuredVehicles) && $featuredVehicles->count() > 0)
            <div class="text-center mt-5">
                <a href="{{ route('gallery') }}" class="btn btn-outline-primary btn-lg px-5">
        ...
        <div class="row g-4 justify-content-center">
            ...
            <div class="col-sm-6 col-lg-3">
                <div class="text-center">
                    ...
                    <h6 style="font-weight: 700; color: #0c2340; margin-bottom: 0.5rem;">{{ $step['title'] }}</h6>
                    <p style="font-size: 0.88rem; color: #64748b; margin: 0; line-height: 1.6;">{{ $step['desc'] }}</p>
                </div>
            </div>
            @endforeach
        </div>
        <div class="text-center mt-4">
            <a href="{{ route('export-process') }}" class="btn btn-primary px-4">
        ...
        <div class="row justify-content-center">
            <div class="col-lg-7 text-center">
                <h2 style="font-family: 'Poppins', sans-serif; font-size: 2.2rem; font-weight: 700; color: #fff; margin-bottom: 1rem;">
                    {{ __('messages.cta_title') ?? 'Ready to Export Your Dream Vehicle?' }}
                </h2>
                <p style="color: rgba(255,255,255,0.7); font-size: 1.05rem; margin-bottom: 2rem; line-height: 1.7;">
                    {{ __('messages.cta_subtitle') ?? 'Get in touch with our expert team today and let us make your vehicle export seamless.' }}
                </p>
                <div class="d-flex gap-3 justify-content-center flex-wrap">
                    <a href="{{ route('contact') }}" class="btn-hero-primary" style="padding: 0.9rem 2.25rem;">
